...
function initCommunityAuth() {
    ...
    // Logout — the header #logoutBtn click is now owned by core.js (it calls
    // the real backend's /api/logout and redirects to login.html). We hook
    // into that real logout via the spc-logout-cleanup event instead of
    // attaching our own click listener, so Community's local state (calls,
    // chat history) is torn down too without double-handling the click.
    document.addEventListener('spc-logout-cleanup', () => {
...
function updateAuthUI() {
    ...
    // If the real backend session is active (auth-client.js), never show the
    // header Login button again — the bridge means currentUser should match
    // it anyway, but this guards against any edge-case mismatch.
    const effectiveUser = currentUser || window.spcCurrentSessionUser;
    if (effectiveUser) {
        if (hlb) { hlb.style.display = 'none'; hlb.style.visibility = 'hidden'; }
        if (ub) { ub.style.display = 'flex'; ub.style.visibility = 'visible'; }
        if (uw) uw.textContent = effectiveUser.username;
        if (headerAvatar && currentUser) headerAvatar.innerHTML = avatarEl(currentUser, 32, '0.7rem');
    } else {
