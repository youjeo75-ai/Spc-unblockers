...
function createGameCard(game) {
    ...
    // when this site embeds their image in an <img> tag. The image is fine
    // and loads everywhere else; it's specifically blocked BECAUSE it's
    // embedded here. referrerpolicy="no-referrer" stops the browser from
    // sending that header at all, which defeats this specific block.
    // The letter fallback (via.placeholder.com used to be the fallback, but
    // that service shut down in 2024) only kicks in if the image still fails
    // even with no referrer sent.
    const thumbContent = game.image
        ? `<img data-src="${game.image}" alt="${game.title}" class="lazy-load" referrerpolicy="no-referrer" onerror="this.remove();">${fallbackLetter}`
        : fallbackLetter;
