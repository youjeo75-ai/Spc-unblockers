...
// Safe localStorage write — a full quota (this site's Community chat
// history/avatars can fill it up) used to throw straight out of an
// unguarded setItem call and silently stop ALL further script execution
// on the page, which is why the games grid could go completely blank with
// no visible error. Every write in this file goes through this now.
function safeSetItem(key, value) {
    try {
        localStorage.setItem(key, value);
        return true;
    } catch (err) {
        console.warn(`[core] localStorage.setItem('${key}') failed (quota likely full):`, err);
        return false;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (clearRecentBtn) {
        clearRecentBtn.addEventListener('click', () => {
            recentlyPlayed = [];
            safeSetItem('spc_recent', JSON.stringify(recentlyPlayed));
            renderRecentlyPlayed();
    ...
    if (ratingContainer) {
        ...
        stars.forEach(star => {
            star.addEventListener('click', () => {
                ...
                ratings[gameId] = val;
                safeSetItem('spc_ratings', JSON.stringify(ratings));
...
// Called via onerror on game thumbnail <img> tags. Only ever runs when an
// image genuinely fails to load (even with no-referrer set) — replaces the
// broken <img> with a single styled letter, in-place, exactly once.
function handleThumbError(img) {
    if (img.dataset.fallbackApplied) return; // guard against repeat error events
    img.dataset.fallbackApplied = '1';
    const letter = img.getAttribute('data-fallback-letter') || '?';
    const span = document.createElement('span');
    span.className = 'thumb-fallback-letter';
    span.style.cssText = "font-size: 2.5rem; font-weight: 800; color: rgba(255,255,255,0.8); text-shadow: 0 4px 15px rgba(0,0,0,0.5); font-family: 'Outfit', sans-serif;";
...
function createGameCard(game) {
    ...
    // Several of these image hosts (azgames.io, agaronline.io, iogamesonly.com,
    // etc.) hotlink-block based on the Referer header — they 403 any request
    // whose Referer isn't their own domain, which is exactly what happens
    // when this site embeds their image in an <img> tag. referrerpolicy
    // ="no-referrer" stops the browser from sending that header, which
    // defeats this specific block, so the real image shows correctly.
    //
    // The letter is a LAST-RESORT fallback for a genuinely broken image
    // link. It must never render alongside a working image, so it's only
    // created by handleThumbError() itself, the moment (and only if)
    // loading actually fails — never pre-rendered into the markup.
    const thumbContent = game.image
        ? `<img data-src="${game.image}" alt="${game.title}" class="lazy-load" referrerpolicy="no-referrer" data-fallback-letter="${game.title.charAt(0)}" onerror="handleThumbError(this)">`
        : `<span class="thumb-fallback-letter" style="font-size: 2.5rem; font-weight: 800; color: rgba(255,255,255,0.8); text-shadow: 0 4px 15px rgba(0,0,0,0.5); font-family: 'Outfit', sans-serif;">${game.title.charAt(0)}</span>`;
...
function toggleFavorite(gameId, btn) {
    ...
    safeSetItem('spc_favorites', JSON.stringify(favorites));
}
...
function initGamePlayer() {
    ...
    if (relatedGrid) {
        ...
        // initLazyLoading() only ever ran after the index.html grids rendered —
        // it was never called here, so these cards' <img data-src="..."> never
        // got copied into a real src, and the images never even attempted to
        // load. attachCardListeners() also needs re-running so favorite
        // buttons on these specific cards work too.
        initLazyLoading();
        attachCardListeners();
    }
...
function saveSettings(settings) {
    safeSetItem('spc_settings', JSON.stringify(settings));
}
